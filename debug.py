...
def main():
  ...
  balls = [{
    ...
    speed: [random.choice([random.uniform(4, 6), random.uniform(-6, -4)]),
            random.uniform(-6, -4)],  # upward start
    colour: white,
    radius: 30
  }]
  ...
  while game_running:
    ...
    for b in balls:
      ...
      # portal collisions (your design preserved)
      if (portal1_pos[0] - b[radius] <= b[pos][0] <= portal1_pos[0] + portal1_width + b[radius] and
      ...
      if (portal2_pos[0] - b[radius] <= b[pos][0] <= portal2_pos[0] + portal2_width + b[radius] and
    ...
    if score % 5 == 0 and score != 0 and last_level_up_score != score:
      ...
      if platform_width <= width // 2:
        platform_width *= 1.05
      if platform_height <= height // 6:
        platform_height *= 1.05
      if platform_speed >= 20:
        platform_speed *= 1.1
      balls.append({
        pos: [width // 2, height // 2],
    ...
    if (portal1_pos[0] - 32 <= 0):
      portal1_dir[0] = 0
    
    if (portal1_pos[0] + portal1_width + 32 >= width):
      portal1_dir[0] = 1
    
    if (portal1_pos[1] - 32 <= 0):
      portal1_dir[1] = 1
    
    if (portal1_pos[1] + portal1_height + 132 >= height):
      portal1_dir[1] = 0
    ...
    if (portal2_pos[0] - 32 <= 0):
      portal2_dir[0] = 1
    
    if (portal2_pos[0] + portal2_width + 2 >= width):
      portal2_dir[0] = 0
    
    if (portal2_pos[1] - 32 <= 0):
      portal2_dir[1] = 0
    
    if (portal2_pos[1] + portal2_height + 132 >= height):
      portal2_dir[1] = 1
