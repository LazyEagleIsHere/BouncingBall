...
def start_screen():
  ...
  while start:
    ...
    show_text_on_screen("Mission: Aim for as high a score as possible", 85, height // 1.2)
    pygame.display.flip()
...
def end_screen(final_score):
  ...
  # safer threading: wait until request finishes
...
def main():
  ...
  max_platform_width = width if developer else width // 2
  ...
  while game_running:
    ...
    for b in balls:
      ...
      # --- WALL COLLISION WITH CLAMP ---
      if b[pos].x - b[radius] <= 0:
        b[pos].x = b[radius]  # clamp inside
        b[speed].x = abs(b[speed].x)
        b[colour] = change_ball_color()
      elif b[pos].x + b[radius] >= width:
      ...
      if b["teleport_cooldown"] > 0:
        ...
        ball_rect = pygame.Rect(
          b[pos].x - int(b[radius] * ball_radius_multi),
          ...
          if rect_from.colliderect(ball_rect):
            if len(portal_rects) > 1:
              ...
              b[pos] = pygame.Vector2(rect_to.centerx,
                                      rect_to.top - int(b[radius] * ball_radius_multi) - 5)
    ...
    pygame.draw.rect(screen, platform_colour,
                     (int(platform_pos[0]), int(platform_pos[1]),
                      platform_width_effective, platform_height))
